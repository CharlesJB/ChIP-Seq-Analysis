...
if (length(argv) == 2) {
	toVenn <- read.table(argv[1], header=TRUE)
	output <- argv[2]

	if (ncol(toVenn) == 1) {
		stop("Need at least 2 samples to create a Venn diagram")
	}
	if (ncol(toVenn) > 5) {
		stop("This script cannot produce Venn diagrams of more than 5 samples at a time")
	}
	library("limma")
	    ...
	    lwd = 2, circle.col, counts.col, show.include, ...) 
	{
		...
		circle.col <- c("darkred","darkblue","darkorange","darkgreen")
	    else 
		circle.col <- par("col")
	    if (length(circle.col) < nsets) 
		circle.col <- rep(circle.col, length.out = nsets)
	    if (missing(counts.col) & nsets == 4) 
		counts.col <- c("darkred","darkblue","darkorange","darkgreen")
		 else
			  counts.col <- par("col") 
		...
		text(xtext[circle], ytext[circle], names[circle], 
				...
				offset = 0.5, col = circle.col[circle], cex = cex, pos = position[circle])
	    }
	...
	writeLines("")
	writeLines("Input file format example:")
	writeLines("Sample1    Sample2    Sample3")
	writeLines("0          1          1")
	writeLines("...        ...        ...")
	writeLines("1          0          1")
	writeLines("")
